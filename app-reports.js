MSyncApp.prototype.renderReports = async function () {
  ...
  <div class="fade-in space-y-6">
    <div class="glass-panel rounded-xl p-4 flex flex-wrap items-end gap-3">
      ...
      <button onclick="app.applyReportRange()" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Apply</button>
      <div class="flex gap-1 ml-2">
        <button onclick="app.setReportPreset('today')" class="px-3 py-2 text-xs bg-gray-100 rounded-lg hover:bg-gray-200">Today</button>
        <button onclick="app.setReportPreset('week')" class="px-3 py-2 text-xs bg-gray-100 rounded-lg hover:bg-gray-200">This Week</button>
        <button onclick="app.setReportPreset('month')" class="px-3 py-2 text-xs bg-gray-100 rounded-lg hover:bg-gray-200">This Month</button>
        <button onclick="app.setReportPreset('lastmonth')" class="px-3 py-2 text-xs bg-gray-100 rounded-lg hover:bg-gray-200">Last Month</button>
        <button onclick="app.setReportPreset('year')" class="px-3 py-2 text-xs bg-gray-100 rounded-lg hover:bg-gray-200">This Year</button>
        <button onclick="app.setReportPreset('all')" class="px-3 py-2 text-xs bg-gray-100 rounded-lg hover:bg-gray-200">All Time</button>
      </div>
      <div class="ml-auto flex gap-2">
        <button onclick="app.exportReportsExcel()" class="px-4 py-2 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700"><i class="fas fa-file-excel mr-2"></i>Export Excel</button>
        <button onclick="app.exportReportsPdf()" class="px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700"><i class="fas fa-file-pdf mr-2"></i>Export PDF</button>
      </div>
    </div>
...
MSyncApp.prototype.loadReportsData = async function () {
  ...
  try {
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="glass-panel rounded-xl p-5"><p class="text-sm text-gray-500">Revenue</p><p class="text-2xl font-bold mt-1">${this.fmt(summary.total_revenue)}</p></div>
      <div class="glass-panel rounded-xl p-5"><p class="text-sm text-gray-500">Sales</p><p class="text-2xl font-bold mt-1">${summary.total_sales}</p></div>
      <div class="glass-panel rounded-xl p-5"><p class="text-sm text-gray-500">Profit</p><p class="text-2xl font-bold mt-1 text-green-600">${this.fmt(summary.total_profit)}</p></div>
      <div class="glass-panel rounded-xl p-5"><p class="text-sm text-gray-500">Margin</p><p class="text-2xl font-bold mt-1">${summary.margin_pct.toFixed(1)}%</p></div>
    </div>

    <div class="glass-panel rounded-xl p-5">
      <h3 class="font-bold mb-3">Sales Over Time</h3>
      <canvas id="reportsChart" height="80"></canvas>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="glass-panel rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-100 font-semibold">Best-Selling Products</div>
        <table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left text-gray-500"><th class="p-3">Product</th><th class="p-3 text-right">Units</th><th class="p-3 text-right">Revenue</th></tr></thead>
        <tbody>${bestSellers.length === 0 ? '<tr><td colspan="3" class="p-4 text-center text-gray-400">No sales in this period</td></tr>' : bestSellers.map(p => `<tr class="border-b border-gray-50"><td class="p-3">${this.esc(p.name)}</td><td class="p-3 text-right">${p.units_sold}</td><td class="p-3 text-right">${this.fmt(p.revenue)}</td></tr>`).join('')}</tbody></table>
      </div>

      <div class="glass-panel rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-100 font-semibold">Profit Margins by Product</div>
        <table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left text-gray-500"><th class="p-3">Product</th><th class="p-3 text-right">Profit</th><th class="p-3 text-right">Margin</th></tr></thead>
        <tbody>${profitMargins.length === 0 ? '<tr><td colspan="3" class="p-4 text-center text-gray-400">No sales in this period</td></tr>' : profitMargins.map(p => `<tr class="border-b border-gray-50"><td class="p-3">${this.esc(p.name)}</td><td class="p-3 text-right">${this.fmt(p.profit)}</td><td class="p-3 text-right">${p.margin_pct.toFixed(1)}%</td></tr>`).join('')}</tbody></table>
      </div>

      <div class="glass-panel rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-100 font-semibold">Sales by Shop</div>
        <table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left text-gray-500"><th class="p-3">Shop</th><th class="p-3 text-right">Sales</th><th class="p-3 text-right">Revenue</th></tr></thead>
        <tbody>${byShop.length === 0 ? '<tr><td colspan="3" class="p-4 text-center text-gray-400">No sales in this period</td></tr>' : byShop.map(s => `<tr class="border-b border-gray-50"><td class="p-3">${this.esc(s.shop_name)}</td><td class="p-3 text-right">${s.sale_count}</td><td class="p-3 text-right">${this.fmt(s.revenue)}</td></tr>`).join('')}</tbody></table>
      </div>

      <div class="glass-panel rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-100 font-semibold">Sales by Salesman</div>
        <table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left text-gray-500"><th class="p-3">Salesman</th><th class="p-3 text-right">Sales</th><th class="p-3 text-right">Revenue</th></tr></thead>
        <tbody>${bySalesman.length === 0 ? '<tr><td colspan="3" class="p-4 text-center text-gray-400">No sales in this period</td></tr>' : bySalesman.map(s => `<tr class="border-b border-gray-50"><td class="p-3">${this.esc(s.salesman_name)}</td><td class="p-3 text-right">${s.sale_count}</td><td class="p-3 text-right">${this.fmt(s.revenue)}</td></tr>`).join('')}</tbody></table>
      </div>
    </div>

    <div class="glass-panel rounded-xl shadow-sm overflow-hidden">
      <div class="p-4 border-b border-gray-100 font-semibold">Low Stock / Restock Needed <span class="text-xs text-gray-400 font-normal">(current snapshot, not date-filtered)</span></div>
      <table class="w-full text-sm"><thead class="bg-gray-50"><tr class="text-left text-gray-500"><th class="p-3">SKU</th><th class="p-3">Product</th><th class="p-3 text-right">Stock</th><th class="p-3 text-right">Threshold</th></tr></thead>
      <tbody>${lowStock.length === 0 ? '<tr><td colspan="4" class="p-4 text-center text-gray-400">Nothing low on stock</td></tr>' : lowStock.map(p => `<tr class="border-b border-gray-50"><td class="p-3">${this.esc(p.sku)}</td><td class="p-3">${this.esc(p.name)}</td><td class="p-3 text-right stock-low px-2 rounded">${p.current_stock}</td><td class="p-3 text-right">${p.low_stock_threshold}</td></tr>`).join('')}</tbody></table>
    </div>`;
...
MSyncApp.prototype.exportReportsExcel = function () {
  if (!this.reportData) { this.showToast('Load a report first', 'warning'); return; }
